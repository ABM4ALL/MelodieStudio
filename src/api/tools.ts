...
export const resizePTY = async (termID: string, rows: number, cols: number): Promise<void> => {
    await request.post("/api/pty/resize", { termID, rows, cols })
    return
}

export const closePTY = async (termID: string): Promise<void> => {
    (await request.post("/api/pty/close", { termID }))
}
