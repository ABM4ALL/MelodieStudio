...
def handle_mermaid():
    ...
    return json.dumps({"mermaid": get_mermaid(file, func_name)})
...
def handle_space_data():
    ...
    return json.dumps(
        [
            {"id": i, "x": random.randint(10, 590), "y": random.randint(10, 590)}
            for i in range(100)
        ]
    )
    # return
...
def run_visualize(
    step_handler: Callable, reset_handler: Callable
) -> Tuple[threading.Thread, MelodieFlask]:
